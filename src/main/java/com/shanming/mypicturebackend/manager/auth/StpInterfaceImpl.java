...
public class StpInterfaceImpl implements StpInterface {
    ...
    public List<String> getPermissionList(Object loginId, String loginType) {
        ...
        // 获取 userId
        User loginUser = (User) StpKit.SPACE.getSessionByLoginId(loginId).get(UserConstants.USER_LOGIN_STATE);
        if (loginUser == null) {
    ...
    /**
     * 判断所有字段都为空” 的方法，通过反射获取对象的所有字段，进行判空：
     * @param object
     * @return
     */
    ...
    private SpaceUserAuthContext getAuthContextByRequest() {
        ...
        //使用hutool的ContentType.JSON.getValue()判断请求参数是不是JSON
        if (ContentType.JSON.getValue().equals(contentType)) {
            //使用hutool的工具类拿到JSON请求体
            String body = ServletUtil.getBody(request);
            //将请求体转化成上下文类
            authRequest = JSONUtil.toBean(body, SpaceUserAuthContext.class);
        }else { //如果不是JSON，则在此项目中一定是get请求
            //使用hutool的ServletUtil.getParamMap(request)拿到请求参数，请求参数就是个map集合
